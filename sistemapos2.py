...
fechahoy = datetime.strptime(str(fechahoy)[:-6], "%Y-%m-%d %H:%M:%S.%f")

def crearTablas(): #crea todas las tablas iniciales
    cursordb.execute('''CREATE TABLE CLIENTE (
                    cedula INTEGER PRIMARY KEY,
                    nombre TEXT,
                    puntos INTEGER,
                    aniver DATE)''') #cliente
    cursordb.execute('''CREATE TABLE INVENTARIO (
                    codigo SERIAL PRIMARY KEY, 
                    nombre TEXT, 
                    cantidad INTEGER, 
                    proveedor INTEGER,
                    precio INTEGER)''') #inventario/productos
    cursordb.execute('''CREATE TABLE CAJERO (
                     cedula INTEGER PRIMARY KEY, 
                     nombre TEXT, 
                     caja INTEGER)''') #cajero
    cursordb.execute('''CREATE TABLE ADMIN (
                     cedula INTEGER PRIMARY KEY, 
                     clave INTEGER)''') #administrador
    cursordb.execute('''CREATE TABLE PROVEEDOR (
                     numero SERIAL PRIMARY KEY, 
                     clave INTEGER)''') #proveedor
    cursordb.execute('''CREATE TABLE PRODPROOV (
                     numprov INTEGER REFERENCES PROVEEDOR(numero),
                     codprod INTEGER REFERENCES INVENTARIO(codigo),
                     PRIMARY KEY(numprov, codprod))''')#proveedor-producto
    cursordb.execute('''CREATE TABLE VENTA (
    ...
    cursordb.execute('''CREATE TABLE CUPONES ( 
                    codcup SERIAL PRIMARY KEY,
                    descuento INTEGER,
                    fechavalida DATE)''') #Cupones
    ...
    cursordb.execute('''INSERT INTO PRODPROOV (numprov, codprod) VALUES
                        ...
                        (2, 4),
                        (1, 5);''')
    cursordb.execute('''INSERT INTO CUPONES (descuento, fechavalida) VALUES
                        (10, CURRENT_DATE + INTERVAL '7 days'),
                        (15, CURRENT_DATE + INTERVAL '14 days'),
                        (20, CURRENT_DATE + INTERVAL '30 days'),
                        (5, CURRENT_DATE + INTERVAL '5 days'),
                        (25, CURRENT_DATE + INTERVAL '20 days'),
                        (12, CURRENT_DATE + INTERVAL '10 days'),
                        (8, CURRENT_DATE + INTERVAL '15 days'),
                        (18, CURRENT_DATE + INTERVAL '25 days'),
                        (30, CURRENT_DATE + INTERVAL '35 days'),
                        (7, CURRENT_DATE + INTERVAL '12 days'); ''')
...
def agregarUsuario(cedula, nombre):
    ...
    if cursordb.fetchone()[0] !=0: 
        ...
        cursordb.execute('''insert into cliente(cedula,nombre,puntos,aniver) values ('{}','{}','{}','{}')'''.format(cedula,nombre,0,fechahoy))
...
# Función para generar un código de cupón único compuesto por números
def crearTablaCupones():
    cursordb.execute("CREATE TABLE IF NOT EXISTS cupones (codcup TEXT PRIMARY KEY, descuento INTEGER, fechavalida DATE)")

def generar_codigo():
    return ''.join(random.choice(string.digits) for _ in range(3))

def crear_cupones(cantidad, porcentaje_descuento, dias_vencimiento):
    for _ in range(cantidad):
        codigo = generar_codigo()
        fecha_vencimiento = (datetime.now() + datetime(days=dias_vencimiento)).strftime("%Y-%m-%d")
        cursordb.execute("INSERT INTO cupones (codigo, porcentaje_descuento, fechaValida) VALUES (?, ?, ?)", (codigo, porcentaje_descuento, fecha_vencimiento))
...
def vender(fecha):
    ...
    while op2 != 0:
        ...
        op2 = int(input())
    if opc == 1 or opc == 3:
        cursordb.execute('''SELECT aniver FROM CLIENTE WHERE cedula = '{}' '''.format(ced))
        aniv = cursordb.fetchone()[0]
        #fechaan = datetime.strptime(aniv, "%Y-%m-%d")
        an = False
        if (fechahoy.day == aniv.day and fechahoy.month == aniv.month and fechahoy.year != aniv.year):
            print("Es el aniversario del cliente, felicitalo e informale del descuento del 40%")
            cursordb.execute("INSERT INTO VENTA (id, total, fecha) VALUES ('{}', '{}', '{}')".format(actventa + 1, int(total*0.6), fecha))
            an = True
        elif not an:
            cursordb.execute('''SELECT puntos FROM CLIENTE WHERE cedula = '{}' '''.format(ced))
            oldpoints = cursordb.fetchone()[0] or 0
            print("Tienes ", oldpoints, " puntos. Desea utilizar sus puntos como descuento, o tiene un cupón de descuento? 1.puntos 2.cupón 3.Ninguno")
            opcDesc = int(input())
            if opcDesc == 1:
                if oldpoints >= total:
                    cursordb.execute("INSERT INTO VENTA (id, total, fecha) VALUES ('{}', '{}', '{}')".format(actventa + 1, 0, fecha))
                    cursordb.execute('''UPDATE CLIENTE SET puntos = '{}' WHERE cedula = '{}' '''.format(oldpoints-total,ced))
                else:
                    cursordb.execute("INSERT INTO VENTA (id, total, fecha) VALUES ('{}', '{}', '{}')".format(actventa + 1, total-oldpoints, fecha))
                    cursordb.execute('''UPDATE CLIENTE SET puntos = '{}' WHERE cedula = '{}' '''.format(0,ced))
            elif opcDesc == 2:
                print("Inserte el código del cupón:")
                codCupon = stdin.readline().strip()
                cursordb.execute('''SELECT codcup FROM CUPONES WHERE codcup = '{}' '''.format(codCupon))
                if cursordb.fetchone()[0] !=0:
                    cursordb.execute('''SELECT fechavalida FROM CUPONES WHERE codcup = '{}' '''.format(codCupon))
                    fechaVencimiento = cursordb.fetchone()[0]
                    fecha_actual = date.today()
                    if  fechaVencimiento < fecha_actual:
                        print("Error: Cupón no válido")
                    else:
                        cursordb.execute('''SELECT descuento FROM CUPONES WHERE codcup = '{}' '''.format(codCupon))
                        descuentoCupon = cursordb.fetchone()[0] or 0
                        print("Tienes un descuento del ", descuentoCupon, "%")
                        cursordb.execute("INSERT INTO VENTA (id, total, fecha) VALUES ('{}', '{}', '{}')".format(actventa + 1, int(total-(total*(descuentoCupon/100))), fecha))
                else:
                    print("Error: El cupón no existe.\n")
